...
if (window.attachEvent && pngua.indexOf('OPERA') == -1 && (pngua.indexOf('MSIE 6') || pngua.indexOf('MSIE 5.5'))) {
    window.attachEvent("onload", PNGuin);
}

function PNGuin() {
    var el, img, imgs, x, w, h, src, sm;
    for (x = 0; x < document.all.length; x++) { // Do background PNGs
        el = document.all[x];
    ...
    for (x = 0; x < imgs.length; x++) { // Do IMG PNGs
        img = imgs[x];
        src = img.src;
        if ((/\.png$/i).test(src) || (/\.png\?/i).test(src)) {
            img.width = img.width;
            img.height = img.height; // Doesn't seem right, does it?
