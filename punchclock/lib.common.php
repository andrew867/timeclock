...
function work_week_begin($local_timestamp = null) {
	...
	if ($local_timestamp == null) $local_timestamp = time() - server_timezone_offset() + timezone_offset();
...
function utm_timestamp($local_timestamp = null) {
	...
	if ($local_timestamp == null) return time() - server_timezone_offset();
...
function local_timestamp($utm_timestamp = null) {
	...
	if ($utm_timestamp == null) $utm_timestamp = time() - server_timezone_offset();
...
function bool($str=null) {
	...
	if ($str && preg_match('/^\s*(no|false|0+)\s*$/i',$str)) return false;
	if ($str) return true;
	return false;
}
